...
const SHELL=['/','/index.html','/connexion.html','/auth.css','/auth.js','/app.css','/app.js','/cloud.css','/cloud.js','/config.js','/supabase.js','/manifest.webmanifest','/icon-192.png','/icon-512.png','/apple-touch-icon.png'];
...
self.addEventListener('fetch',event=>{const u=new URL(event.request.url);if(event.request.method!=='GET'||u.origin!==self.location.origin)return;
  ...
  if(event.request.mode==='navigate'){event.respondWith(caches.open(CACHE).then(async c=>(await c.match(u.pathname==='/connexion.html'?'/connexion.html':'/index.html'))||fetch(event.request)));return}
