...
settingsView=function(){
 ...
 if(profileSection==='account'){
  const u=cloud.user;
  return head+`<section class="card"><h2>Les infos de ton compte.</h2><div class="account-detail"><span>Adresse e-mail</span><b>${u?esc(u.email||'Non renseignée'):'Aucun compte connecté'}</b></div><div class="account-detail"><span>Connexion</span><b>${u?'Google · mot de passe géré par Google':'Mode local'}</b></div><div class="account-detail"><span>Membre depuis</span><b>${u?.created_at?esc(new Date(u.created_at).toLocaleDateString('fr-FR')):'—'}</b></div></section><div style="margin-top:20px">${cloudCard()}</div>`;
 }
